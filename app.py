...
def is_promo_or_bogo(text):
    ...
    # Check for dollar signs (indicating promotional pricing in name)
    if '
...
def transform_row(row):
    ...
    # Check if name contains promo/BOGO/dollar signs
...
def process_csv(uploaded_file):
    ...
    # Track seen Product IDs and duplicate keys for deduplication
    seen_product_ids = {}  # product_id -> row_index
    seen_duplicates = {}  # (name, size, classification) -> row_index
    ...
    for idx, row in enumerate(reader, start=2):  # Start at 2 (row 1 is header)
        ...
        if transformed:
            ...
            if product_id and product_id in seen_product_ids:
                # Duplicate Product ID found - replace the earlier one
                old_idx = seen_product_ids[product_id]
                filtered_rows[old_idx] = transformed  # Replace with newer entry
                skipped_rows.append((idx, f"Duplicate Product ID (kept newer): {product_id}", transformed.get("Name", "N/A")))
            else:
                # Check for likely duplicates (same name, size, classification)
                dup_key = (
                ...
                if dup_key in seen_duplicates and dup_key[0]:  # Only if name is not empty
                    # Likely duplicate found - replace the earlier one
                    ...
                    if dup_key[0]:  # Only track if name exists
